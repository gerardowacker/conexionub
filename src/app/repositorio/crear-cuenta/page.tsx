...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await post('/register', { email, password, displayName });
    ...
    setSubmitting(false);
    setError((data as any)?.error || 'No se pudo crear la cuenta.');
  };

  return (
    <Container
      id="register"
      crumb={["Inicio",<Link key="register" href="crear-cuenta">Crear cuenta</Link>
      ]}
    >
      <div className={styles.card}>
        ...
        <form onSubmit={handleSubmit} className={styles.form} noValidate>
          ...
          <input type="password" name="password" className={styles.input} placeholder="Contraseña (mín. 8)" required />
          <input type="password" name="confirm"  className={styles.input} placeholder="Confirmar contraseña" required />
