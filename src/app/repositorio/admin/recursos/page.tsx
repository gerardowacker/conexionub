        ...
        <div className={styles.controlsRow}>
          <div className={styles.controlsBox}>
            ...
            <div className={styles.centerControls} style={{ flex: 1 }}>
              <select value={selectedCollection ?? ''} onChange={e => { const v = e.target.value || null; setSelectedCollection(v); }}>
            ...
            <div className={styles.rightControls}>
              <button className={styles.secondaryBtn} onClick={() => loadResources(true, selectedCollection)}>Refrescar lista</button>
            </div>
          </div>
        </div>
        ...
        <div className={styles.listWrapper}>
          ...
          {resourcesLoading ? (
            ...
            <ResourceList
              ...
              onSelectAction={(res) => { setSelected(res); }}
            />
          )}
          ...
          <div className={treeStyles.modalBackdrop} role="dialog">
            <div className={treeStyles.modal}>
              <div className={treeStyles.modalHeader}>
                ...
                <button className={treeStyles.closeBtn} onClick={() => setSelected(null)} aria-label="Cerrar">×</button>
              </div>

              <div className={treeStyles.modalBody}>
                <ResourceForm ref={formRef} initial={selected === 'new' ? null : selected} onSavedAction={() => { loadResources(true, selectedCollection); setSelected(null); }} />
              </div>

              <div className={treeStyles.modalFooter}>
                <button className={treeStyles.btnSecondary} onClick={() => setSelected(null)}>Cancelar</button>
                <button className={treeStyles.btnPrimary} onClick={() => { if (formRef.current && typeof formRef.current.submit === 'function') formRef.current.submit(); }}>Guardar</button>
              </div>
            </div>
          </div>
        )}
